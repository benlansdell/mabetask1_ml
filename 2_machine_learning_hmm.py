...
def compute_metrics(y, pred):
    ...
    cc = np.corrcoef(y, pred)[0,1]
    if np.isnan(cc):
        cc = 0
    return [re, pr, auc, cc]

def run_gridsearch_rf_xgb_cv(X_train, y_train, groups_train, X_val, y_val, params = None, refit = False):
    ...
    models = {'logreg': [sklearn.linear_model.LogisticRegression(), [], {'C': np.logspace(-2,2,5)}, []],
            'rf': []}
    ...
    for name in models:
        ...
        clf = GridSearchCV(model, params, cv = gss, scoring = 'f1', verbose = 2, n_jobs = 10)
